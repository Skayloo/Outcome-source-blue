...
/**
 * Remove a message. In a server channel it is tombstoned ("message deleted" stays visible,
 * so moderation is not silent); in a DM the server PURGES the row, so the message is dropped
 * from the list entirely — there is nothing left to stand in for.
 */
export function deleteMessage(payload: ChatDeletedPayload): void {
  messagesStore.setState((prev) => {
    ...
    const updatedList = payload.purged === true
      ? channelMessages.filter((msg) => msg.id !== payload.message_id)
      : channelMessages.map((msg) =>
          msg.id === payload.message_id ? { ...msg, deleted: true, content: "" } : msg,
        );
...
/** Soft-delete every message authored by a user across all channels (account deletion).
 *  Kept in the array with deleted:true so replies to them render a "Deleted message" placeholder. */
export function markUserMessagesDeleted(userId: number): void {
  messagesStore.setState((prev) => {
    const updatedMessages = new Map(prev.messagesByChannel);
    let changed = false;
    for (const [channelId, list] of prev.messagesByChannel) {
      if (!list.some((m) => m.user.id === userId && !m.deleted)) continue;
      updatedMessages.set(
        channelId,
        list.map((m) => (m.user.id === userId ? { ...m, deleted: true } : m)),
      );
      changed = true;
    }
    return changed ? { ...prev, messagesByChannel: updatedMessages } : prev;
  });
}
...
/**
 * Invalidate the "loaded" flag for every channel so the next time one is opened it
 * refetches its latest history from REST. Used on server switch: the WebSocket is scoped
 * to ONE server at a time, so while you were on another server you did NOT receive this
 * server's live messages — the cached history can be missing anything sent in the meantime
 * (the exact "message sent from another client doesn't show until refresh" bug). Cached
 * messages are kept for an instant render; the refetch then reconciles to current state.
 */
export function invalidateLoadedChannels(): void {
  messagesStore.setState((prev) =>
    prev.loadedChannels.size === 0
      ? prev
      : { ...prev, loadedChannels: new Set<number>() },
  );
}
