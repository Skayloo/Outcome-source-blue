...
export interface DmChannel {
  ...
  /** The peer's read marker — own messages up to it show ✓✓. */
  readonly peerReadUpTo: number;
}
...
/** The DM peer's read marker moved — own messages up to it show ✓✓. */
export function advancePeerRead(channelId: number, messageId: number): void {
  dmStore.setState((s) => ({
    channels: s.channels.map((c) =>
      c.channelId === channelId && c.peerReadUpTo < messageId
        ? { ...c, peerReadUpTo: messageId }
        : c,
    ),
  }));
}
