...
export interface AuthState {
  ...
  /** A stored session was rejected by the server → stop waiting, show the login form. */
...
/** The server rejected a restored session — the boot splash must give way to the login form. */
export function markSessionInvalid(): void {
  authStore.setState((prev) => ({ ...prev, sessionInvalid: true }));
}
