 ...
 * channel that lives on a different server (e.g. the voice dock's "open" button while you
 * are browsing another space). switchServer() clears channels and the fresh READY normally
 * auto-selects the first text channel; a pending id overrides that once it's loaded.
 */
let pendingChannelId: number | null = null;
export function setPendingChannel(id: number | null): void { pendingChannelId = id; }
/** Read and clear the pending channel id. */
export function takePendingChannel(): number | null {
...
/** Another participant's read marker moved — own messages up to it show ✓✓. */
export function advanceOthersRead(channelId: number, messageId: number): void {
  channelsStore.setState((prev) => {
    const existing = prev.channels.get(channelId);
    if (existing === undefined || existing.othersReadUpTo >= messageId) return prev;
    const channels = new Map(prev.channels);
    channels.set(channelId, { ...existing, othersReadUpTo: messageId });
