...
async function ensureRunning(ctx: AudioContext): Promise<void> {
  ...
  log.info("AudioContext not running — resuming before wiring the graph", { state: ctx.state });
  try {
    await ctx.resume();
  } catch (err) {
    log.warn("AudioContext resume failed — the processed track may start silent", err);
  }
}
